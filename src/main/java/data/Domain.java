...
//                        String[] result = curr_rule.result;
    ...
    public List<List<Tuple>> groupByKey(List<HashMap<Integer, Tuple>> domains, List<Tuple> rules,
                                        List<HashMap<String, ArrayList<Integer>>> convert_domains) {

        HashMap<Integer, Tuple> domain = null;
        HashMap<String, ArrayList<Integer>> convert_domain = null;
        ...
        for (int i = 0; i < size; i++) {
            ...
            while (c_iter.hasNext()) {
                Entry<String, ArrayList<Integer>> entry = c_iter.next();
    ...
    public static ArrayList<DistanceInfo> insertSort(ArrayList<DistanceInfo> disList, DistanceInfo insertInfo, int MAXSIZE) {
        ...
        if (disList.size() > MAXSIZE)
            disList.remove(size - 1);//删除
        return disList;
    }

    public void smoothOutlierToGroup(List<List<Tuple>> domain_outlier, List<List<HashMap<Integer, Tuple>>> Domain_to_Groups,
                                     HashMap<Integer, String[]> dataSet, List<HashMap<String, ArrayList<Integer>>> convert_domains,
                                     HashMap<Integer, ArrayList<String>> dataSet_noIgnor) {
        ...
        for (int i = 0; i < Domain_to_Groups.size(); i++) {
            ...
            HashMap<String, ArrayList<Integer>> convert_groups = convert_domains.get(i);
...
//                HashSet<String> flagBox = new HashSet<>();//存放已经比较过的tupleContext
                Iterator<Entry<String, ArrayList<Integer>>> iter = convert_groups.entrySet().iterator();
