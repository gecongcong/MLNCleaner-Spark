        ...
        try {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (part_i < partitionNum - 1 && i == number * (part_i + 1) - 1) {
                    dataBw[part_i].flush();
                    dataBw[part_i].close();
                    groundRules(newHeader, dataName, part_dataList.get(part_i), rules, part_i);
                    part_i++;
                } else if (part_i == partitionNum - 1 && i == size - 1) {
                    dataBw[part_i].flush();
                    dataBw[part_i].close();
...
//                dataBw[k].close();
    ...
    public static void partitionMLN(String dirtyfile, String cleanfile, ArrayList<String> rules, int partitionNum, String dataName) {
        ...
        HashMap<String, String> map = combineRulesFile(clean_fileURL, dirty_fileURL, dataName);
        String newcleanFile = cleanfile.replaceAll(".csv", "-hasID.csv");
        main.Main.setLineID(cleanfile, newcleanFile);
...
//            br.readLine();
     ...
     * @throws IOException
     */
    public void formatEvidence(String outFile, ArrayList<Integer> newHeaderList) throws IOException {
        String content = "";
